...
@Component({
  ...
  template: `
    <div class="py-8 max-w-4xl mx-auto">
      <h1 class="text-3xl font-bold text-slate-900 mb-8">Cart</h1>
      <!-- Loading -->
      ...
      <div class="grid gap-6 lg:grid-cols-3">
            ...
            @for (item of items(); track item.product.id) {
              <div class="bg-white rounded-xl shadow-md p-4 flex gap-4">
                <img
                  ...
                  class="size-24 object-contain rounded-lg bg-slate-50"
                />
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-xl shadow-md p-6 sticky top-24">
              ...
              <button appButton class="w-full mt-6 bg-linear-to-r from-emerald-600 to-sky-900">
                Proceed to Checkout
              </button>
              <button
                appButton
                variant="ghost"
                class="w-full mt-2 text-red-500 hover:text-red-600 hover:bg-red-50"
                (click)="onClearCart()"
              >
                Clear Cart
              </button>
      ...
      <div class="flex flex-col items-center justify-center h-64 bg-white rounded-xl shadow-md">
        ...
        <a appButton routerLink="/products" class="bg-linear-to-r from-emerald-600 to-sky-900">Continue Shopping</a>
      </div>
      }
    </div>
  `,
